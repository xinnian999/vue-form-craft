...
2. **类型提示**: 使用 TypeScript 时,切换 UI 库后类型提示保持一致
3. **插槽适配**: 不同 UI 库的插槽名称可能不同,需要在适配器中进行映射
4. **事件适配**: 事件名称和参数可能不同,需要在适配器中统一转换为协议定义的格式
5. **外部化依赖**: formora 将所有 UI 库设置为 peerDependencies,不会打包进库中
6. 部分组件的样式适配，需要在src/style.scss中进行穿透，注意不要使用deep选择器。
7. 适配的时候，不要使用嵌套的三目运算，因为不好维护。
8. 完成适配，要搜索项目所有使用到的组件，替换为新适配的组件。
