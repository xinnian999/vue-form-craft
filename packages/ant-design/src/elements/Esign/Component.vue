<template>
  <template v-if="formInstance.read || value">
    <div>
      ...
      <a-button v-if="!formInstance.read" :disabled @click="handleClear" type="primary"
        >重写</a-button
      >
    </div>
  </template>
  ...
  <div v-else class="fm-esign">
    ...
    <div>
      <a-button v-if="oldValue" :disabled @click="value = oldValue" plain type="primary"
        >取消</a-button
      >
      <a-button :disabled @click="handleReset" plain type="primary">清空</a-button>
      <a-button :disabled @click="handleGenerate" type="primary">保存</a-button>
    </div>
  </div>
</template>
