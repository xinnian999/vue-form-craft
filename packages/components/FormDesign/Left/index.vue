<template>
  <div :class="ns('form-design-left')" :style="{ overflow: drawerVisible ? 'hidden' : 'auto' }">
    <div :class="ns('template')">
      <Button :class="ns('template-button')" size="small" @click="drawerVisible = true">{{
      ...
      >
        <Space wrap direction="vertical" alignment="start">
          <Button :key="name" v-for="{ name, schema } in templates" @click="useTemplate(schema)">
            {{ name }}
          </Button>
        </Space>
      </Drawer>
...
import { useComponents, useDesignInstance, useElements, useLang, useLocale } from '@form-magic/core'
import { getRandomId, ns } from '@form-magic/core'

const { Button, Drawer, Space } = useComponents()
...
const templates = designInstance.templates
